...
class Perception:
    ...
    def add_sensor(self, sensor: Sensor):
        ...
        self.perception_magnitude_costmap = self.create_costmap(costmap_type='perception')
        self.noise_costmap = self.create_costmap(costmap_type='noise')
        logger.success("Sensor added and perception magnitude and noise costmaps updated")
    ...
    def get_perception_magnitude(self, x_true: np.ndarray):
        ...
        magnitudes = np.array([sensor.get_sensor_magnitude(x_true) for sensor in self.sensors])
    ...
    def get_epsilon(self, noise: float, mode: str):
        ...
        if mode == "robust":
            ...
            logger.error(f"Unknown mode '{mode}' Supported modes: 'robust', 'probabilistic'")
            logger.info("Use robust mode.")
            epsilon = 4 * noise
        return epsilon

    def calculate_safety_margin(self, noise: float, u_nominal: np.ndarray, mode: str = "robust"):
    ...
    def create_costmap(self, costmap_type: str):
        ...
        if costmap_type == 'noise':
            ...
            costmap = self.get_perception_magnitude_batched(jnp.array(pos))  # shape (N,)
        else:
